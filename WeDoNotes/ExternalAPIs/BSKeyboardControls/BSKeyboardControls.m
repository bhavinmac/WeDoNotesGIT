...
- (id)initWithFields:(NSArray *)fields
{
    if (self = [super initWithFrame:CGRectMake(0.0f, 0.0f, 320.0f, 44.0f)])
    {
        ...
        UILabel* titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(150.0 ,10.0f, 100, 21.0f)];
...
//        [titleLabel setText:@"Title"];
        ...
        [self setDoneButton:[[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Done", @"BSKeyboardControls", @"Next button title.")
...
- (void)doneButtonPressed:(id)sender
{
    ...
    {
        [self.delegate keyboardControlsDonePressed:self.activeField];
    }
}
...
- (void)selectPreviousField
{
    ...
    {
        index -= 1;
        UITextField *field = [self.fields objectAtIndex:index];
        ...
        if ([self.delegate respondsToSelector:@selector(keyboardControlsPreviousPressed:)])
        {
            [self.delegate keyboardControlsPreviousPressed:self.activeField];
        }
    }
}
